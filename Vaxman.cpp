...
// Edited by: Kanan Aliyev
// EA Virtual Intership Task 2
// Project: Vaxman

// if my code is being checked, I marked the additions as: // Change
...
// Change
// I added the color into the arrays so that it is easier to continue
...
// Change
float time_elapsed; // this variable will hold the elapsed time for spreading the infection later
vector<float*> monsters; // a vector of monsters because this will make it easier to handle them ahead
...
void init(void) {
	...
	// Change
	// filling the field with the monsters
...
void resetGame() {
	...
	// Change
	// resetting includes the colors obviously
	...
	// Change
	// And most importantly, saving the elapsed time here
...
void gameLogic() {
	...
	// Change
	// Variable to hold the end of the monsters vector
	vector<float*>::iterator last_monster = monsters.end();

	// Change
	// looping through the vector and checking, deleting if a monster was eaten
	for (auto first = monsters.begin(); first != monsters.end(); ++first) {
		float* first_monster = *first;

		int monsterX = (int)(first_monster[0]);
		int monsterY = (int)(first_monster[1]);

		if (pacmanX == monsterX && pacmanY == monsterY) {
			over = false;
			last_monster = first;
			delete first_monster;
			break;
		}
	}

	// Change
	// Seeing if the last monster is dead, if so, erasing it and ending the game basically
	if (last_monster != monsters.end()) {
		monsters.erase(last_monster);
	}
...
void resultsDisplay() {
	if (points == 106) {
		...
		while (*message) { glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, *message++); }
		...
		while (*message) { glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, *message++); }
	}
...
void welcomeScreen() {
	...
	while (*message) { glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, *message++); }
	message = "Vaxman - by Kanan Aliyev";
	glColor3f(1, 1, 1);
...
// Change
// Custom function which applies the next rule which is to spread the "infection"
void infectionSpread(void) {
	// variables holding the times
	int old_timer;
	float new_timer;

	// and here, doing what I mentioned earlier, creating the timer for use
	old_timer = glutGet(GLUT_ELAPSED_TIME);
	new_timer = (old_timer - time_elapsed) / 1000.0;
	glutPostRedisplay();

	// resetting it
	if (new_timer < 2) { return; }
	time_elapsed = old_timer;
	int n = monsters.size();

	// Pushing new monsters in, in other words, spreading
	for (int i = 0; i < n; ++i) {
...
void display() {
	...
	// Change
	// Calling the custom and modified methods
	keyOperations();
	...
	if (replay) {
		if (!over) {
			...
			// Change
			// looping through again like in the gameLogic function to draw the monsters
			for (auto first = monsters.begin(); first != monsters.end(); ++first) {
				float* first_monster = *first;
				updateMonster(first_monster);
				drawMonster(first_monster[0], first_monster[1], first_monster[3], first_monster[4], first_monster[5]);
			}
...
int main(int argc, char** argv) {
	...
	glutCreateWindow("Vaxman - by Kanan Aliyev");
